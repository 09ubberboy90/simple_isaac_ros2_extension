...
class ThrowHandler(BaseSample):
    ...
    def setup_scene(self):
        ...
        self.spawn_obj("/Spawned/", position=[0.5,0.0,0.025], param_name="target") # target must be second to spawn

        self.spawn_obj("/Spawned/", position=[1,0.065,0.025])
        self.spawn_obj("/Spawned/", position=[1,0.0,0.025])
        self.spawn_obj("/Spawned/", position=[1,-0.065,0.025])
        self.spawn_obj("/Spawned/", position=[1,-0.0335,0.075])
        self.spawn_obj("/Spawned/", position=[1,0.0335,0.075])
        self.spawn_obj("/Spawned/", position=[1,0.0,0.135])

        return

    def spawn_obj(self, path, position=[0, 0, 0], rotation = [1,0,0,0], param_name=None):
        self.counters+=1
        if not param_name:
